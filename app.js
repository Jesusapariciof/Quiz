...
function quizResult() {
  ...
  resultBox.querySelector(".total-percentage").innerHTML =
...
function startTimer(time) {
  ...
  function timer() {
    ...
    let sec = time % 60;
    if (time > 60) {
      timeCount.textContent = min + "min" + sec + "s";
    } else if (time <= 60) {
      timeCount.textContent = time + "s";
      timeCount.style.color = "red"; //change the color to red if there is only a minute left
    }
    time--;
    if (questionCounter === quiz.length) {
